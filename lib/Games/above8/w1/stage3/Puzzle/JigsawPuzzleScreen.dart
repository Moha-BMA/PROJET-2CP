import 'dart:math';
import 'package:chainage1/Wilayas_stages/above%208/w1/stage_screen1.dart';
...
import 'package:chainage1/Games/above8/w1/stage3/Settings.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> with SingleTickerProviderStateMixin {
  final String puzzleImage = 'assets/assetsgamesunderabove/above8/wilaya1/stage3/puzzle/puzzle_image.png';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _isFirstNextPress = !(prefs.getBool('W1_stage3_next_pressed') ?? false);
      initializePuzzle();
    });
  }
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/above8/wilaya1/stage3/puzzle/puzzle_piece_${index + 1}.png'),
  ...
  Offset calculateGridPosition(int row, int col) {
    ...
    return Offset(puzzleLeft + col * pieceWidth, puzzleTop + row * pieceHeight);
  }
  ...
  void checkGameCompletion() {
    if (correctlyPlaced.every((placed) => placed)) {
      ...
      secondStarEarned = _totalTimeInSeconds <= 50;
      thirdStarEarned = _totalTimeInSeconds <= 35;
  ...
  void saveStageCompletion() async {
    try {
      final prefs = ref.read(sharedPreferencesProvider);
      int previousStars = prefs.getInt('W1_stage3_stars') ?? 0;

      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W1_stage3_stars', totalStarsEarned);
        await prefs.setBool('W1_stage3_completed', true);

        int currentProgress = prefs.getInt('S_wilaya1') ?? 1;
        if (currentProgress < 2 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya1', 2);
        }

        final starsNotifier = ref.read(starsProvider.notifier);
        starsNotifier.addStars(totalStarsEarned - previousStars);

        print("Stage completion saved: Wilaya 1, Stage 3 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W1_stage3_completed', true);
      }
    } catch (e) {
              ...
              children: [
                ...
                if (_isTrophyContainerVisible)
                  GestureDetector(
                    onTap: () async {
                      final prefs = ref.read(sharedPreferencesProvider);
                      await prefs.setBool('W1_stage3_next_pressed', true);
                      setState(() {
                              ...
                              AnimatedBuilder(
                                ...
                                builder: (context, child) {
                                  return Transform.scale(
                                    ...
                                    child: Image.asset(
                                      'assets/assetsTrophyspace/images/trophy1_col.png',
                                      width: size.width * (200 / 917),
                                      height: size.height * (300 / 412),
                                      fit: BoxFit.contain,
                                    ),
                                  );
                                },
