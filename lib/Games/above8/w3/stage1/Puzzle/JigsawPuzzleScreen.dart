...
import 'package:chainage1/Wilayas_stages/above 8/w3/stage_screen3.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> {
  final String puzzleImage = 'assets/assetsgamesunderabove/above8/wilaya3/stage1/puzzle/puzzle_image.png';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/above8/wilaya3/stage1/puzzle/puzzle_piece_${index + 1}.png'),
  ...
  void saveStageCompletion() async {
    try {
      ...
      int previousStars = prefs.getInt('W3_stage1_stars') ?? 0;

      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W3_stage1_stars', totalStarsEarned);
        await prefs.setBool('W3_stage1_completed', true);
        ...
        print("Stage completion saved: Wilaya 3, Stage 1 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W3_stage1_completed', true);
      }
    } catch (e) {
