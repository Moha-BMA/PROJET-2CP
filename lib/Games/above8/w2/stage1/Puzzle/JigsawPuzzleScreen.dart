...
import 'package:chainage1/Games/above8/w2/stage1/loading1.dart';
import 'package:chainage1/Games/above8/w2/stage1/Settings.dart';
import 'package:flutter/material.dart';
import 'dart:async';
import 'GridPainter.dart';
import 'PuzzlePiece.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:chainage1/Wilayas_stages/above 8/w2/stage_screen2.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> {
  final String puzzleImage = 'assets/assetsgamesunderabove/above8/wilaya2/stage1/puzzle/puzzle_image.png';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/above8/wilaya2/stage1/puzzle/puzzle_piece_${index + 1}.png'),
  ...
  void saveStageCompletion() async {
    try {
      ...
      int previousStars = prefs.getInt('W2_stage1_stars') ?? 0;

      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W2_stage1_stars', totalStarsEarned);
        await prefs.setBool('W2_stage1_completed', true);

        int currentProgress = prefs.getInt('S_wilaya2') ?? 1;
        if (currentProgress < 2 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya2', 2);
        }

        final starsNotifier = ref.read(starsProvider.notifier);
        starsNotifier.addStars(totalStarsEarned - previousStars);

        print("Stage completion saved: Wilaya 2, Stage 1 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W2_stage1_completed', true);
      }
    } catch (e) {
