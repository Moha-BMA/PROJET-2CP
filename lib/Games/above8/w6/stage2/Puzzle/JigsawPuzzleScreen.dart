...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> {
  final String puzzleImage = 'assets/assetsgamesunderabove/above8/wilaya6/stage2/puzzle/puzzle_image.jpg';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/above8/wilaya6/stage2/puzzle/puzzle_piece_${index + 1}.jpg'),
  ...
  void saveStageCompletion() async {
    try {
      ...
      int previousStars = prefs.getInt('W6_stage2_stars') ?? 0;

      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W6_stage2_stars', totalStarsEarned);
        await prefs.setBool('W6_stage2_completed', true);

        int currentProgress = prefs.getInt('S_wilaya6') ?? 1;
        if (currentProgress < 3 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya6', 3);
        }

        final starsNotifier = ref.read(starsProvider.notifier);
        starsNotifier.addStars(totalStarsEarned - previousStars);

        print("Stage completion saved: Wilaya 6, Stage 2 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W6_stage2_completed', true);
      }
    } catch (e) {
