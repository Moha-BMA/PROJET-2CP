...
import 'package:chainage1/Games/above8/w5/stage1/loading1.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> {
  final String puzzleImage = 'assets/assetsgamesunderabove/above8/wilaya5/stage1/puzzle/puzzle_image.jpg';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/above8/wilaya5/stage1/puzzle/puzzle_piece_${index + 1}.jpg'),
  ...
  void saveStageCompletion() async {
    try {
      ...
      int previousStars = prefs.getInt('W5_stage1_stars') ?? 0;

      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W5_stage1_stars', totalStarsEarned);
        await prefs.setBool('W5_stage1_completed', true);
        ...
        print("Stage completion saved: Wilaya 5, Stage 1 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W5_stage1_completed', true);
      }
    } catch (e) {
