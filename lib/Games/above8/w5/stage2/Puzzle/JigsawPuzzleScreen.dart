...
import 'package:chainage1/Games/above8/w5/stage2/loading2.dart';
import 'package:chainage1/Games/above8/w5/stage2/Settings.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> {
  final String puzzleImage = 'assets/assetsgamesunderabove/above8/wilaya5/stage2/puzzle/puzzle_image.jpeg';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/above8/wilaya5/stage2/puzzle/puzzle_piece_${index + 1}.jpg'),
  ...
  void checkGameCompletion() {
    if (correctlyPlaced.every((placed) => placed)) {
      ...
      thirdStarEarned = _totalTimeInSeconds <= 40;
  ...
  void saveStageCompletion() async {
    try {
      ...
      int previousStars = prefs.getInt('W5_stage2_stars') ?? 0;

      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W5_stage2_stars', totalStarsEarned);
        await prefs.setBool('W5_stage2_completed', true);

        int currentProgress = prefs.getInt('S_wilaya5') ?? 1;
        if (currentProgress < 3 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya5', 3);
        }

        final starsNotifier = ref.read(starsProvider.notifier);
        starsNotifier.addStars(totalStarsEarned - previousStars);

        print("Stage completion saved: Wilaya 5, Stage 2 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W5_stage2_completed', true);
      }
    } catch (e) {
