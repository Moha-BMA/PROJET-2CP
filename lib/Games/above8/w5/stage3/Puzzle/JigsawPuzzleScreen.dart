import 'dart:math';
import 'package:chainage1/Games/above8/w5/stage3/loading3.dart';
import 'package:chainage1/Wilayas_stages/above 8/w5/stage_screen5.dart';
import 'package:chainage1/app_state.dart';
import 'package:chainage1/Games/above8/w5/stage3/Settings.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> {
  final String puzzleImage = 'assets/assetsgamesunderabove/above8/wilaya5/stage3/puzzle/puzzle_image.jpg';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/above8/wilaya5/stage3/puzzle/puzzle_piece_${index + 1}.jpg'),
  ...
  void checkGameCompletion() {
    if (correctlyPlaced.every((placed) => placed)) {
      ...
      secondStarEarned = _totalTimeInSeconds < 70;
      thirdStarEarned = _totalTimeInSeconds < 45;
  ...
  void saveStageCompletion() async {
    try {
      ...
      int previousStars = prefs.getInt('W5_stage3_stars') ?? 0;

      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W5_stage3_stars', totalStarsEarned);
        await prefs.setBool('W5_stage3_completed', true);

        int currentProgress = prefs.getInt('S_wilaya5') ?? 1;
        if (currentProgress < 4 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya5', 4);
        }

        final starsNotifier = ref.read(starsProvider.notifier);
        starsNotifier.addStars(totalStarsEarned - previousStars);

        print("Stage completion saved: Wilaya 5, Stage 3 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W5_stage3_completed', true);
      }
    } catch (e) {
