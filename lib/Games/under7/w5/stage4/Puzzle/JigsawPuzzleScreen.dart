import 'dart:math';
import 'package:chainage1/Games/under7/w5/stage4/Settings.dart';
import 'package:chainage1/Wilayas_stages/under 7/w5/stage_screen5.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> with SingleTickerProviderStateMixin {
  final String puzzleImage = 'assets/assetsgamesunderabove/under7/wilaya5/stage4/puzzle/puzzle_image.jpg';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/under7/wilaya5/stage4/puzzle/puzzle_piece_${index + 1}.jpg'),
  ...
  void saveStageCompletion() async {
    try {
      ...
      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W5_stage4_stars', totalStarsEarned);
        await prefs.setBool('W5_stage4_completed', true);
        int currentProgress = prefs.getInt('S_wilaya5') ?? 1;
        if (currentProgress < 2 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya5', 2);
        }
        final starsNotifier = ref.read(starsProvider.notifier);
        starsNotifier.addStars(totalStarsEarned - previousStars);
        print("Stage completion saved: Wilaya 5, Stage 4 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W5_stage4_completed', true);
      }
    } catch (e) {
