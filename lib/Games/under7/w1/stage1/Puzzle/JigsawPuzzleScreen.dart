...
import 'package:chainage1/Games/under7/w1/stage1/loading1.dart';
import 'package:flutter/material.dart';
import 'package:chainage1/Games/under7/w1/stage1/Settings.dart';
import 'dart:async';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> {
  final String puzzleImage = 'assets/assetsgamesunderabove/under7/wilaya1/stage1/puzzle/puzzle_image.png';
  ...
  // Variable to store the puzzle grid position
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/under7/wilaya1/stage1/puzzle/puzzle_piece_${index + 1}.png'),
  ...
  void checkGameCompletion() {
    ...
    if (correctlyPlaced.every((placed) => placed)) {
      ...
      secondStarEarned = totalTimeInSeconds <= 60;  // 1 minute

      // Third star: earned based on a faster time threshold (e.g., 2 minutes)
      thirdStarEarned = totalTimeInSeconds <= 50;  // 50 seconds
  ...
  void saveStageCompletion() async {
    try {
      ...
      int previousStars = prefs.getInt('W1_stage1_stars') ?? 0;
      ...
      if (totalStarsEarned > previousStars) {
        ...
        await prefs.setInt('W1_stage1_stars', totalStarsEarned);

        // Mark stage as completed
        await prefs.setBool('W1_stage1_completed', true);
        ...
        if (currentProgress < 2 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya1', 2); // Unlock stage 2
        }
        ...
        print("Stage completion saved: Wilaya 1, Stage 1 - $totalStarsEarned stars");
      } else {
