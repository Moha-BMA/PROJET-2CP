import 'dart:math';
import 'package:chainage1/Games/under7/w6/stage3/loading3.dart';
...
class _JigsawPuzzleScreenState extends ConsumerState<JigsawPuzzleScreen> with SingleTickerProviderStateMixin {
  final String puzzleImage = 'assets/assetsgamesunderabove/under7/wilaya6/stage3/puzzle/puzzle_image.jpg';
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < columns; col++) {
        ...
        PuzzlePiece piece = PuzzlePiece(
          id: index,
          imageProvider: AssetImage('assets/assetsgamesunderabove/under7/wilaya6/stage3/puzzle/puzzle_piece_${index + 1}.jpg'),
  ...
  void saveStageCompletion() async {
    try {
      ...
      if (totalStarsEarned > previousStars) {
        await prefs.setInt('W6_stage3_stars', totalStarsEarned);
        await prefs.setBool('W6_stage3_completed', true);
        int currentProgress = prefs.getInt('S_wilaya6') ?? 1;
        if (currentProgress < 2 && totalStarsEarned > 0) {
          await prefs.setInt('S_wilaya6', 2);
        }
        final starsNotifier = ref.read(starsProvider.notifier);
        starsNotifier.addStars(totalStarsEarned - previousStars);
        print("Stage completion saved: Wilaya 6, Stage 3 - $totalStarsEarned stars");
      } else {
        await prefs.setBool('W6_stage3_completed', true);
      }
    } catch (e) {
