...
class OpenedStageGroup2 extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<OpenedStageGroup2> createState() => _OpenedStageGroup2();
}

class _OpenedStageGroup2 extends ConsumerState<OpenedStageGroup2> {
  int _visibleStars = 0;
  ...
  @override
  void didUpdateWidget(OpenedStageGroup2 oldWidget) {
          ...
          switch (widget.stageNumber) {
            case 1:
              Navigator.push(
                ...
                int updatedStars = prefs.getInt('W2_stage1_stars') ?? widget.starstate;
                if (updatedStars != widget.starstate) {
                  ref.invalidate(w2Stage1StarsProvider);
                }
              });
              break;
            case 2:
              Navigator.push(
                ...
                final prefs = ref.read(sharedPreferencesProvider);
                int updatedStars = prefs.getInt('W2_stage2_stars') ?? widget.starstate;
                if (updatedStars != widget.starstate) {
                  ref.invalidate(w2Stage2StarsProvider);
                }
              });
              break;
            default:
